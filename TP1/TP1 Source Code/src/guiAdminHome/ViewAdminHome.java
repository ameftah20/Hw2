 ...
 * JavaFX GUI widgets that enable an admin to perform admin functions. This page
 * contains a number of buttons that have not yet been implemented. What has
 * been implemented may not work the way the final product requires and there
 * maybe defects in this code.
 * 
 * The class has been written using a singleton design pattern and is the View
 * portion of the Model, View, Controller pattern. The pattern is designed that
 * the all accesses to this page and its functions starts by invoking the static
 * method displayAdminHome. No other method should attempt to instantiate this
 * class as that is controlled by displayAdminHome. It ensure that only one
 * instance of class is instantiated and that one is properly configured for
 * each use.
 * 
 * Please note that this implementation is not appropriate for concurrent
 * systems with multiple users. This Baeldung article provides insight into the
 * issues: https://www.baeldung.com/java-singleton
 * </p>
 * 
 * <p>
...
public class ViewAdminHome {
	...
	*/
	 ...
	 * to cause the Admin Home page to be displayed.
	 * 
	 ...
	 * changeable fields using the displayAdminHome method.
	 * </p>
	 * 
	 */
	private ViewAdminHome() {
		...
		theRootPane.getChildren().addAll(label_PageTitle, label_UserDetails, button_UpdateThisUser, line_Separator1,
				label_NumberOfInvitations, label_NumberOfUsers, line_Separator2, label_Invitations,
				label_InvitationEmailAddress, text_InvitationEmailAddress, combobox_SelectRole, button_SendInvitation,
				line_Separator3, button_ManageInvitations, button_SetOnetimePassword, button_DeleteUser,
				button_ListUsers, button_AddRemoveRoles, line_Separator4, button_Logout, button_Quit);
