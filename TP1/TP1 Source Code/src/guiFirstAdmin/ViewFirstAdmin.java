...
public class ViewFirstAdmin {
	...
	private ViewFirstAdmin() {
		...
		button_AdminSetup.setDisable(true);  // keep disabled until form validates

		text_AdminUsername.setTextFormatter(new TextFormatter<String>(
		    change -> change.getControlNewText().length() <= 16 ? change : null));

		text_AdminPassword1.setTextFormatter(new TextFormatter<String>(
		    change -> change.getControlNewText().length() <= 32 ? change : null));

		text_AdminPassword2.setTextFormatter(new TextFormatter<String>(
		    change -> change.getControlNewText().length() <= 32 ? change : null));
	...
	public static void setAdminSetupEnabled(boolean enabled) { button_AdminSetup.setDisable(!enabled); }
	public static void showFormMessage(String text, boolean isError) {
	    label_PasswordsDoNotMatch.setTextFill(isError ? javafx.scene.paint.Color.RED : javafx.scene.paint.Color.GREEN);
	    label_PasswordsDoNotMatch.setText(text);
	}
